...
- **Mutação de `status` disputada por 2 caminhos concorrentes usa `UPDATE ... SET status=novo WHERE id=? AND status=<esperado>` (checar `rowsAffected`), não dirty-check do JPA (`save()` de entidade carregada sem lock).** O `save()` gera `UPDATE ... WHERE id=?` sem guard → **last-writer-wins**, sem sinal ao perdedor. Ex.: `cancelarPorParticipante` faz certo; o par check-in/cancelamento do mesmo ingresso caiu em dirty-check (aceito como last-writer-wins pois o duplo check-in é guardado por `UNIQUE(ingresso_id)` e o dinheiro por `pagamentos.status` sob lock — mas é a exceção, não a regra). Se a `architecture.md` **declara** "transição condicional serializa a corrida", o Revisor confirma que a query é de fato `UPDATE...WHERE status=?` e não um `save()`. *(aprendizado Sprint 5B, CR-5B-02; generaliza ADR-T07/T15)*
...
- **Publisher com `RabbitTemplate` opcional loga a perda em `WARN`, nunca `DEBUG`.** O padrão "injeção opcional (`@Autowired(required=false) setRabbitTemplate`) + no-op se `null`" existe só para subir o contexto sob perfil `test` (H2, `RabbitAutoConfiguration` excluída, sem broker). **Em produção o perfil não é `test`** → o `RabbitTemplate` sempre existe; um `null` ali significa que a autoconfig do broker falhou e o **evento de domínio seria perdido em silêncio** (saga/reembolso não disparam). O ramo `null` faz no-op (não derruba a operação local já commitada) **mas loga `WARN` com a chave de negócio** (`inscricaoId`/`eventId`) — a perda precisa ser observável para reconciliação. *(aprendizado Sprint 5B, CR-5B-01; reapareceu 3× — `PagamentoAprovadoPublisher` S4, `PedidoCriadoPublisher`/`InscricaoCanceladaPublisher` 5B)*
